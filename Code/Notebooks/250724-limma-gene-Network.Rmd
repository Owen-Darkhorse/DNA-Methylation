...
## Bell-shaped Corr Distribution Near 0
```{r}
geneSamp <- sample(colnames(logX), 200)

S <- var(logX[, geneSamp]) ## Covariance matrix of the top 200 genes
glassoRes <- glasso(S, rho = 0.1,
           nobs = nrow(logX),
           maxit = 100)
penalizedCov <- glassoRes$w
penalizedCorrelation <- penalizedCov / sqrt(outer(diag(penalizedCov), diag(penalizedCov)) )


par(mfrow = c(2,2))
hist(cor(logX[, top200dmrs]), main = "Distribution of correlations before penalty",
breaks = "Scott", col = "pink")

hist(S, main = "Distribution of covariance before penalty", col = "pink", breaks = "Scott")

hist(glassoRes$w, main = "Distribution of penalized covariance",
breaks = "Scott", col = "pink")

hist(penalizedCorrelation, main = "Distribution of penalized correlations", col = "pink", breaks = "Scott")


hist(solve(S), main = "Distribution of nverse penalized covariance", col = "pink", breaks = "Scott")

hist(solve(glassoRes$w), main = "Distribution of inverse penalized covariance", col = "pink", breaks = 200)
```

```{r}
w_inv <- solve(glassoRes$w)
hist(w_inv, main = "Distribution of inverse penalized covariance", col = "pink", breaks = 200)


## After normalizing
w_inv_diag <- diag(w_inv)
w_inv_std <- w_inv / sqrt(outer(w_inv_diag, w_inv_diag))
hist(w_inv_std, main = "Distribution of standardized inverse penalized covariance", col = "pink", breaks = 200)

off_diag <- upper.tri(w_inv_std)
off_diag <- w_inv_std[off_diag]
hist(off_diag, main = "Partial Correlation")

off_diag <- upper.tri(w_inv)
off_diag <- w_inv[off_diag]

hist(off_diag, main = "Inverse of Estimated covariance Matrix")
```
